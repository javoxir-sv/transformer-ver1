...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model:int, seq_len:int, dropout:float):
       ...
       div_term = torch.exp(torch.arange(0, d_model, dtype=torch.float).float() * (-np.log(10000) / d_model))
...
class MultiHeadAttention(nn.Module):
    def __init__(self, d_model:int, h:int, dropout:float):
        ...
        assert d_model % h == 0, "error: d_model % h != 0"
    ...
    def attention(query, key, value, mask, dropout:nn.Dropout):
        ...
        attention_values = (query @ key.transpose(-2, -1)) / np.sqrt(d_k)
        if mask is not None:
            attention_values.masked_fill_(mask == 0, -1e9)
        attention_values = attention_values.softmax(dim=-1) # (batch, h, seq_len, seq_len)
    ...
    def forward(self, q, k, v, mask):
        ...
        x, self.attention_values = MultiHeadAttention.attention(query, key, value, mask, self.dropout)
...
# loop for the EncoderBlock
class EncoderNx(nn.Module):
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x, endcoder_output, src_mask, tgt_mask):
        x = self.residual_connections[0](x, lambda x: self.self_attention_block(x, x, x, tgt_mask))
        x = self.residual_connections[1](x, lambda x: self.cross_attention_block(x, endcoder_output, endcoder_output, src_mask))
        x = self.residual_connections[2](x, self.feed_forward_block)
        return x


class DecoderNx(nn.Module):
    def __init__(self, layers: nn.ModuleList):
        super().__init__()
        self.layers = layers
        self.norm = LayerNormalization()
...
# LINEAR LAYERS
class ProjectctionLayer(nn.Module):
    def __init__(self, d_model:int, vocab_size:int):
        super().__init__()
...
class Transformer(nn.Module):
    def __init__(self, 
                 ...
                 projection_layer: ProjectctionLayer):
        super().__init__()
...
def build_transformer(src_vocab_size:int, tgt_vocab_size:int, src_seq_len:int, tgt_seq_len:int, d_model:int=512, N:int=6, h:int=8, dropout:float=0.1, d_ff:int=2048) -> Transformer:
    ...
    for _ in range (N):
        ...
        cross_self_attention = MultiHeadAttention(d_model, h, dropout)
        feed_forward = FeedForwardLayer(d_model, d_ff, dropout)
        decoder_block = DecoderBlock(decoder_self_attention, cross_self_attention, feed_forward, dropout)
        decoder_blocks.append(decoder_block)

    # The Encoder and Decoder parts itself
    encoder = EncoderNx(nn.ModuleList(encoder_blocks))
    decoder = DecoderNx(nn.ModuleList(decoder_blocks))

    # THe projection layer - the linear layers after the decoder
    projection_layer = ProjectctionLayer(d_model, tgt_vocab_size)
