...
from config import get_weights_filepath, get_config
...
def greedy_decode(model, source, source_mask, tokenizer_src, tokenizer_tgt, max_len, device):
    ...
    # precompute the encoder output and reuseit
    ...
    while True:
        ...
        _, next_word = torch.max(prob, dim=-1) # greedy search btw
...
def run_validation(model, validation_ds, tokenizer_src, tokenizer_tgt, max_len, device, print_msg, num_examples=2): # global_state, writer
    model.eval()
    count = 0
...
def get_ds(config):
    ...
    print(f"Max length of src senteces: {max_len_src}")
    print(f"Max length of tgt senteces: {max_len_tgt}")
...
def train_model(config):
    ...
    for epoch in range(initial_epoch, config['num_epochs']):
        ...
        for batch in batch_iterator:
            ...
            projection_output = model.project(decoder_output) # (batch, seq_len, get_vocab_size)

            label = batch['label'].to(device) # (batch, seq_len)
            # (batch, seq_len, get_vocab_size) --> (batch * seq_len, get_vocab_size)
